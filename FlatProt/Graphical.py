...
def create_rectangle_between_2_points(x1, y1, x2, y2, color, thickness):
    ...
    normalized_orthogonal_dx, normalized_orthogonal_dy = (
        get_normalized_orthogonal_vector(dx, dy)
    )
    # normalized_dx,normalized_dy = get_normalized_vector(dx,dy)

    # four vertices of the rectangle
    x1a, y1a = (
        x1 + normalized_orthogonal_dx * thickness / 2,
        y1 + normalized_orthogonal_dy * thickness / 2,
    )
    x1b, y1b = (
        x1 - normalized_orthogonal_dx * thickness / 2,
        y1 - normalized_orthogonal_dy * thickness / 2,
    ...
    x2a, y2a = (
        x2 + normalized_orthogonal_dx * thickness / 2,
        y2 + normalized_orthogonal_dy * thickness / 2,
    )
    x2b, y2b = (
        x2 - normalized_orthogonal_dx * thickness / 2,
        y2 - normalized_orthogonal_dy * thickness / 2,
...
def create_arrow_line_between_2_points(x1, y1, x2, y2, color, thickness, opacity):
    ...
    dir_line = dwg.polyline(
        [(x1a, y1a), (x2, y2), (x1b, y1b)], stroke="black", fill=color, opacity=opacity
    )
    return dir_line
...
def create_helix_between(x1, y1, x2, y2, color, thickness):
    ...
    remaining_length = get_coord_distance(
        next_module_coords[0], next_module_coords[1], x2, y2
    )
    # start element:
    dwg.add(create_helix_start_element(x1, y1, x2, y2, color, thickness, rect_witdh))

    while remaining_length >= (module_size + rect_witdh):
        module, next_module_coords = create_helix_module(
            next_module_coords[0],
            next_module_coords[1],
            x2,
            y2,
            color,
            thickness,
        ...
        dwg.add(module)
        next_module_coords = (
            next_module_coords[0] - (normalized_orthogonal_dx * thickness / 2),
            next_module_coords[1] - (normalized_orthogonal_dy * thickness / 2),
        )
        remaining_length = get_coord_distance(
            next_module_coords[0], next_module_coords[1], x2, y2
        )
    ...
    end_element_coords = (
        end_element_start[0],
        end_element_start[1],
        end_element_start[0] + (normalized_dx * rect_witdh),
        end_element_start[1] + (normalized_dy * rect_witdh),
    )
    dwg.add(
        dwg.polygon(
            [
                (end_element_coords[0], end_element_coords[1]),
                (end_element_coords[2], end_element_coords[3]),
                (x2, y2),
            ],
            fill=color,
            stroke="black",
...
def create_helix_start_element(x1, y1, x2, y2, color, thickness, rect_witdh):
    ...
    dwg = svgwrite.Drawing(profile="full", size=("100%", "100%"))
    start_triangle = dwg.polygon(
        [(x1, y1), (xr1, yr1), (xr2, yr2)], fill=color, stroke="black"
    )
...
def create_helix_module(x1, y1, x2, y2, color, thickness, rect_witdh):
    ...
    xr2, yr2 = xr1 + (normalized_dx * rect_witdh), yr1 + (normalized_dy * rect_witdh)
    xr3, yr3 = (
        xr2 + (normalized_orthogonal_dx * thickness),
        yr2 + (normalized_orthogonal_dy * thickness),
    )
    xr4, yr4 = xr3 + (normalized_dx * rect_witdh), yr3 + (normalized_dy * rect_witdh)
    front_rect = dwg.polygon(
        [(xr1, yr1), (xr2, yr2), (xr4, yr4), (xr3, yr3)], fill=color, stroke="black"
    )

    xr5, yr5 = (
        xr4 - (normalized_orthogonal_dx * thickness),
        yr4 - (normalized_orthogonal_dy * thickness),
    )
    xr6, yr6 = xr5 + (normalized_dx * rect_witdh), yr5 + (normalized_dy * rect_witdh)
    behind_rect = dwg.polygon(
        [(xr3, yr3), (xr4, yr4), (xr6, yr6), (xr5, yr5)], fill=color, stroke="black"
    )
    dwg.add(behind_rect)
    dwg.add(front_rect)
...
def create_simple_helix_line(
    x1, y1, x2, y2, color, thickness, rect_widht, cross_width, opacity
):
    dwg = svgwrite.Drawing(profile="full", size=("100%", "100%"))
...
def get_line_points(
    x1, y1, x2, y2, thickness, rect_widht, cross_width, min_ending_line
):
    if x1 == x2 and y1 == y2:
        return [(x1, y1), (x2, y2)]
    ...
    first_down = move_point_double_vec(
        points[-1],
        norm_ort_vec,
        -((thickness / 2) - (rect_widht / 2)),
        norm_vec,
        cross_width / 2,
    )
    ...
    distance = (
        get_coord_distance(dist_reference_point[0], dist_reference_point[1], x2, y2)
        - min_ending_line
    )

    number_of_segments = distance / cross_width
    for i in range(0, int(number_of_segments)):
        up_point = move_point_double_vec(
            points[-1], norm_ort_vec, thickness - rect_widht, norm_vec, cross_width / 2
        )
        points.append(up_point)
        # back down:
        down_point = move_point_double_vec(
            points[-1],
            norm_ort_vec,
            -(thickness - rect_widht),
            norm_vec,
            cross_width / 2,
        )
...
def move_point_double_vec(start_point, vector1, distance1, vector2, distance2):
    final = (
        start_point[0] + (vector1[0] * distance1) + (vector2[0] * distance2),
        start_point[1] + (vector1[1] * distance1) + (vector2[1] * distance2),
    )
    return final
...
def testing():
    ...
    dwg = svgwrite.Drawing(
        "connecting_helix_test.svg", profile="full", width="100%", height="100%"
    )
