...
class Structure_Database:
    ...
    def __init__(self, foldseek_executable, tmp_dir, user_db: bool = False):
        ...
        self.foldseek_db = self.load_package_file(
            self.db_directory + ".foldseek_db", "db"
        )  # SCOP SF database dir
            ...
            if fixed_id != None:
                try:
                    ...
                    if fixed_id not in fam_info["ID"].values:
                        raise ValueError(
                            f"{fixed_id} is not a valid ID in the id list of the database."
                        )
                    sf_row = fam_info.loc[fam_info["ID"] == fixed_id]
                    fixed_representative_name = sf_row["representative"].iloc[0]
                    try:
                        matched_row = df[
                            df["target"] == fixed_representative_name + ".pdb"
                        ].iloc[0]
                        print(
    ...
    def initial_and_fixed_Sf_rot_region(
        ...
        filename_without_ext = ".".join(filename.split(".")[:-1])
        sf_aligned_rot_pdb = (
            self.tmp_dir + "/" + filename_without_ext + f"_sf-{sf}_aligned_rot.pdb"
        )
        sf_fixed_rot_pdb = (
            self.tmp_dir + "/" + filename_without_ext + f"_sf-{sf}_fixed_rot.pdb"
        )
    ...
    def init_db_from_summary_file(SCOP_summary_file, db_outfile):
        ...
        summary["rotation_matrix"] = [numpy_matrix_to_string(init_matrix)] * len(
            summary
        )
    ...
    def set_manual_id_rotation(self, id, numpy_rotatoin_matrix, manual=True):
        if not isinstance(
            numpy_rotatoin_matrix, np.ndarray
        ) or numpy_rotatoin_matrix.shape != (3, 3):
        ...
        db.loc[db["ID"] == id, "rotation_matrix"] = numpy_matrix_to_string(
            numpy_rotatoin_matrix
        )
        if manual:
            db.loc[db["ID"] == id, "rotation_type"] = "manual"
        db.to_csv(self.db_info_tsv, sep="\t", index=False)
        print(
            f"Successfully changed fixed rotation for SF: {id} to {numpy_rotatoin_matrix}"
        )
        return True
    ...
    with open(output_file, "w") as out:
        for line in lines:
            if line.startswith(("ATOM", "HETATM")):
                ...
                out.write(
                    f"{line[:30]:<30}{x_new:8.3f}{y_new:8.3f}{z_new:8.3f}{line[54:]}"
                )
            else:
                out.write(line)
